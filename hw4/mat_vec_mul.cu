...
int main(int argc, char** argv){
  ...
  //for (long p = 1024; p <= vec_size; p *= 2) 
  {
    ...
    for (long i = 0; i < m*n; i++) a[i] = i+1;    //rand() %100000;
    for (long i = 0; i < n; i++) b[i] = i+1;     //rand() %100000;
    ...
    double bandwidth = ((m*n + n + n_blocks + n_blocks*n_threads_per_block) *sizeof(double))/(1000000000*time_vec_mul_gpu); 
