...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  // TODO: See instructions below
    for (long k_out = 0; k_out < n; k_out+=BLOCK_SIZE) {
      for (long j_out = 0; j_out < k; j_out+=BLOCK_SIZE) {
        for (long i_out = 0; i_out < m; i_out+=BLOCK_SIZE) {
            for (long kk = k_out; kk < k_out+BLOCK_SIZE; kk++) {
              for (long jj = j_out; jj < j_out+BLOCK_SIZE; jj++) {
                for (long ii = i_out; ii < i_out+BLOCK_SIZE; ii++) {
                  double A = a[kk + n*ii];
                  double B = b[ii + k*jj];
                  double C = c[kk + m*jj];
                  C = C + A*B;
